...
fn downsample_array(arr: &Array2<f32>, max_dim: usize) -> Array2<f32> {
    let (rows, cols) = arr.dim();
    ...
    let src = arr.as_slice().unwrap_or(&[]);
    let mut dst = Array2::<f32>::zeros((dst_rows, dst_cols));
...
pub fn encode_with_header_downsampled(arr: &Array2<f32>, max_dim: usize) -> Result<Vec<u8>> {
    let downsampled = downsample_array(arr, max_dim);
    encode_with_header(&downsampled)
}
...
mod tests {
    ...
    fn test_downsample_no_op_when_small() {
        ...
        let result = downsample_array(&arr, 200);
    ...
    fn test_downsample_large() {
        ...
        let result = downsample_array(&arr, 2048);
        assert_eq!(result.dim(), (2048, 2048));
    }

    #[test]
    fn test_downsample_rectangular() {
        let arr = Array2::from_shape_fn((8192, 4096), |(r, c)| (r + c) as f32);
        let result = downsample_array(&arr, 2048);
        let (h, w) = result.dim();
        assert!(h <= 2048);
        assert!(w <= 2048);
