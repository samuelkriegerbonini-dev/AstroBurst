...
pub fn resolve_input(path: &Path) -> Result<ResolvedInput> {
    if path.is_dir() {
        ...
        fits.sort();
        if fits.is_empty() {
            bail!("No supported image files found in directory {:?}", path);
        }
...
fn is_asdf_path(p: &Path) -> bool {
    p.extension()
        .map(|ext| ext.eq_ignore_ascii_case("asdf"))
        .unwrap_or(false)
}

fn is_supported_image(p: &Path) -> bool {
    is_fits_path(p) || is_asdf_path(p)
}
...
fn extract_from_zip(zip_path: &Path) -> Result<ResolvedInput> {
...
mod tests {
    ...
    fn test_is_fits_path() {
        ...
        assert!(!is_fits_path(Path::new("data.asdf")));
    }

    #[test]
    fn test_is_asdf_path() {
        assert!(is_asdf_path(Path::new("roman_image.asdf")));
        assert!(is_asdf_path(Path::new("data.ASDF")));
        assert!(!is_asdf_path(Path::new("data.fits")));
    }

    #[test]
    fn test_is_supported_image() {
        assert!(is_supported_image(Path::new("data.fits")));
        assert!(is_supported_image(Path::new("data.asdf")));
        assert!(!is_supported_image(Path::new("data.png")));
    }
