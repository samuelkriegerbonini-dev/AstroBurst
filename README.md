...
**Measurement round (unreleased).** Since 0.6.1 the Analysis tab also offers aperture controls with a curve of growth and EE50/EE80 radii, a batch photometry table with the apertures drawn on the image, calibrated region fluxes (Jy, AB, mag/arcsec^2, arcsec^2, RA/Dec, sky position angle), an elliptical surface-brightness profile with R50/R80/R90 and the Petrosian radius, spectral line measurement with a Gaussian fit, contours with export to polygon regions, a Targets panel that overlays an RA/Dec list, separation and position angle for line regions, a compass and scale bar, a pixel table, time-series aperture photometry with a differential light curve, and spectrum-linked channel navigation that publishes a cube channel as a measurable image.

**Science round 2 (unreleased).** The Analysis tab also draws a built-in rest-frame line list (hydrogen Balmer, Paschen and Brackett lines, [OII], [OIII], [NII], [SII], He I, H2, [FeII], CO, HCN, HCO+, [CII] and HI) on the spectrum at a typed redshift or systemic velocity, in the frame of the velocity axis, and a click on a line sets the rest wavelength of the line measurement. The displayed spectrum exports as CSV with provenance (vacuum wavelength, the displayed axis, flux unit, Jy when calibrated, ICRS position of the pixel or region centre), and a comparison section overlays the spectra of every included region and the clicked pixel, normalised or offset, with a legend and a joined CSV. An observation geometry panel reports, for the loaded file and for every frame of a time series, BJD_TDB and HJD_UTC of mid-exposure (leap-second table, TT/TDB, light time to the barycentre within 1 s), local sidereal time, hour angle, altitude, azimuth, Kasten-Young airmass, parallactic angle and the Sun and Moon altitude, phase and separation, with the site and target read from the header (source shown) or typed once and remembered; the light curve can use BJD_TDB as its time axis and exports it in the CSV. A line region on a cube becomes a position-velocity diagram: the cube is sampled along the slit with a chosen step and averaging width without loading it whole, offsets are centred on the slit in arcseconds when a WCS exists, the spectral axis follows the panel's wavelength, frequency or velocity mode, the panel plots the intensity-weighted ridge (median-subtracted) with the peak channel per offset, and the product is a 2D FITS with an OFFSET axis and the cube's native spectral axis that opens in any viewer. A measurement log at the end of the tab records every number the tab produces with its provenance (UTC timestamp, file, measured image, DQ handling, unit, parameters, region) and exports the session as CSV.
...
- **DS9** is the reference FITS viewer. AstroBurst matches most of its inspection surface: stretch and limit algorithms, colormaps, frames, a coordinate grid, region shapes with DS9 `.reg` exchange, per-region statistics and a Gaia catalog overlay. DS9 is ahead on frame management and blinking, image-server integration, 3D rendering, and interoperability through SAMP and XPA, none of which AstroBurst has.
