...
import { Crosshair, Star, Loader2, Eye, EyeOff, Globe, Compass } from "lucide-react";
...
export default function PlateSolvePanel({
  ...
  const { plateSolve, getApiKey, getWcsInfo } = useBackend();
  ...
  const [wcsInfo, setWcsInfo] = useState(null);

  useEffect(() => {
    getApiKey()
      .then((r) => setHasApiKey(!!r?.key || !!r?.apiKey))
  ...
  }, [filePath, scaleLow, scaleHigh, plateSolve, getWcsInfo]);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="bg-zinc-950/50 rounded-lg border border-zinc-800/50 overflow-hidden">
        ...
        <div className="px-3 py-2 space-y-2">
          ...
          {!solveResult && activeWcs && activeWcs.center_ra !== undefined && (
            <div className="grid grid-cols-2 gap-1.5 text-[10px]">
              <div className="bg-zinc-900/80 rounded px-2 py-1.5">
                <div className="text-zinc-500">Center RA</div>
                <div className="text-zinc-300 font-mono">{formatRA(activeWcs.center_ra)}</div>
              </div>
              <div className="bg-zinc-900/80 rounded px-2 py-1.5">
                <div className="text-zinc-500">Center Dec</div>
                <div className="text-zinc-300 font-mono">{formatDec(activeWcs.center_dec)}</div>
              </div>
              {activeWcs.pixel_scale_arcsec && (
                <div className="bg-zinc-900/80 rounded px-2 py-1.5">
                  <div className="text-zinc-500">Pixel Scale</div>
                  <div className="text-zinc-300 font-mono">{activeWcs.pixel_scale_arcsec.toFixed(3)}"/px</div>
                </div>
              )}
              {activeWcs.field_of_view_w_arcmin && (
                <div className="bg-zinc-900/80 rounded px-2 py-1.5">
                  <div className="text-zinc-500">FOV</div>
                  <div className="text-zinc-300 font-mono">
                    {activeWcs.field_of_view_w_arcmin.toFixed(1)}' x {activeWcs.field_of_view_h_arcmin?.toFixed(1)}'
                  </div>
                </div>
              )}
            </div>
